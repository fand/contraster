...
app.directive('contrast', function() {
  return {
    scope: {
      hue: '=',
      text: '@'
    ...
    link: function(scope, element, attr) {
      var bg, box, fix, update, validate;
      box = element[0].querySelector('.box');
      bg = util.getBg('.box');
      fix = 's';
      update = function(option) {
        var hsl_array, result;
        ...
        if (result != null) {
          ...
          return scope.l = result.hsl[2] * 100.0;
        } else {
          scope.is_valid = false;
          scope.hsl_code = '-';
          scope.rgb_code = '-';
          return scope.contrast_new = '-';
        }
      };
...
},{"./../app/bower_components/angular/angular":2,"./contrast.coffee":3,"./util.coffee":4}],2:[function(require,module,exports){
/**
...
var CANDIDATES, c, darkOnLight, fgOnBg, formatHSL, hsl2rgb, lightOnDark, param2hsl, relativify, rgb2L, util;
...
CANDIDATES = (function() {
  ...
  results = [];
  for (c = i = 0; i <= 100; c = ++i) {
    results.push(c / 100.0);
  }
  return results;
})();

rgb2L = function(rgb) {
  var L, br, gr, ref, rr, srgb;
  srgb = rgb.map(function(x) {
    return x / 255.0;
  });
  ref = srgb.map(relativify), rr = ref[0], gr = ref[1], br = ref[2];
  L = 0.2126 * rr + 0.7152 * gr + 0.0722 * br;
  return L;
};

relativify = function(v) {
  if (v <= 0.03928) {
    return v / 12.92;
  } else {
...
lightOnDark = function(bg, hsl, ratio, param_name) {
  var L2ratio;
  L2ratio = function(L_bg, L_fg) {
    return (L_fg + 0.05) / (L_bg + 0.05);
  };
  return fgOnBg(bg, hsl, ratio, param_name, L2ratio);
};

darkOnLight = function(bg, hsl, ratio, param_name) {
  var L2ratio;
  L2ratio = function(L_bg, L_fg) {
    return (L_bg + 0.05) / (L_fg + 0.05);
  };
  return fgOnBg(bg, hsl, ratio, param_name, L2ratio);
};

fgOnBg = function(bg, hsl, ratio, param_name, L2ratio) {
  var L_bg, L_new, contrast_new, diff, diff_min, hsl_new, hue, i, l, len, p, p_nearest, r, readables, ref, rgb_new, s;
  hsl = (ref = formatHSL(hsl), hue = ref[0], s = ref[1], l = ref[2], ref);
  L_bg = rgb2L(bg);
  readables = CANDIDATES.filter(function(c) {
    var L, hsl_candidate, rgb;
  ...
  p_nearest = -1;
  diff_min = 2.0;
  for (i = 0, len = readables.length; i < len; i++) {
    r = readables[i];
    diff = Math.abs(r - p);
    if (diff < diff_min) {
      p_nearest = r;
...
param2hsl = function(hsl, param, param_name) {
  switch (param_name) {
    case 's':
      return [hsl[0], param, hsl[2]];
    case 'l':
      return [hsl[0], hsl[1], param];
  }
};
...
hsl2rgb = function(hsl) {
  var h, hue, l, m, rgb, s, x;
  hue = hsl[0], s = hsl[1], l = hsl[2];
  h = hue / 60.0;
 ...
 * @param  {number[]} color
 */
var array2code, array2str, code2array, css, getBg, hsl2rgb, rgb2hsl, str2array;

code2array = function(code) {
  var codes, s3, s6;
  s6 = code.match(/#?([a-fA-F0-9]{6})/);
  if (s6) {
    codes = s6[1].match(/(..)/g);
    return codes.map(function(c) {
      return Number('0x' + c, 16);
    });
  }
...
rgb2hsl = function(rgb) {
  var b, c, g, h, hh, l, mmax, mmin, r, ref, s, x;
  ref = (function() {
    var i, len, results;
    results = [];
    for (i = 0, len = rgb.length; i < len; i++) {
      x = rgb[i];
      results.push(x / 255.0);
    }
    return results;
